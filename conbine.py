...
def add_dynamic():
    ...
    for item in dnm:
        ...
        current_price = safe_convert(item['price']['present_price'])
...
def add_static():
    ...
    for item in stt:
        ...
        params = safe_convert(json.dumps(item['specifications'], ensure_ascii = False, encoding = 'utf-8'))
        ...
        url = safe_convert(item['website'])
        keyowrd = safe_convert(item['keyword'])
        images = safe_convert(item['image_urls'])
...
def safe_convert(raw):
    if (type(raw) == type(u'') or type(raw) == type('')):
        return raw.replace("'", "''")
